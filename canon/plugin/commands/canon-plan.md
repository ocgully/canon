...
2. Walks the user through: approach, decomposition sketch, dependencies (auto-suggested via Mercator if present), risks + mitigations, implementation order / parallelism hints, rollout + rollback (required for user-facing specs), open questions.
3. Each open question becomes a Hopewell node tagged `open-question` (if Hopewell is installed).
